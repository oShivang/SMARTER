...
def main():
    ...
    print(f"Target Datasets for LLM Baseline: {dataset_list}", flush=True)
    ...
    # H100 parallel VRAM > 24GB gets batch size 64, else 16
    use_parallel_loading = (total_vram > 24)
    hf_batch_size = 64 if use_parallel_loading else 16
    
    print("="*60, flush=True)
    print(f"SYSTEM PROFILE (LLM-ONLY BENCHMARK):", flush=True)
    ...
    try:
        ...
        for ds_name in dataset_list:
            ...
            # Use appropriate full evaluation sample count
            ...
            # Map baseline using optimized batched Hugging Face generation
            baseline_ds = run_llm_baseline_hf(full_dataset, baseline_config_ds, llm, llm_tokenizer, hf_batch_size)
            
            # Inject score metrics required by score/evaluate scripts
            ...
            # Save results & companion metrics
        ...
        print("Tearing down LLM model instance and releasing memory...", flush=True)
        if 'llm' in locals():
            del llm
        clear_gpu_memory()
        
    print("\n" + "="*50)
    print("✅ LLM BASELINE RUN COMPLETE - GENERATING FINAL REPORT CARD")
    print("="*50 + "\n")
