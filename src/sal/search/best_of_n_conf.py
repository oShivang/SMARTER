...
def best_of_n_conf(x, config: Config, llm: LLM, prm: PRM):
    ...
    completion_likelihood = [[] for _ in range(len(x["problem"]))]
    completion_likelihood_mean = [[] for _ in range(len(x["problem"]))]
    completion_tokenprobs_mean = [[] for _ in range(len(x["problem"]))]
