...
def _beam_search(batch_of_prompts, config: Config, llm: LLM, prm: PRM) -> tuple[list[Beam], int, list]:
    ...
    for i in tqdm(range(config.num_iterations), desc="Beam search iterations"):
        ...
        # scores = prm.score(prompts, completions) # used for tracking scores for sequential steps
        # agg_scores = [ # used for selecting top beams
        ...
        if config.filter_duplicates:
            ...
            conf_agg_scores = [conf_agg_scores[i] for i in unique_beam_dict.values()]
...
def beam_search_conf(examples, config: Config, llm: LLM, prm: PRM):
    problems = examples["problem"]
    beam_results, total_tokens, prm_scores = _beam_search(problems, config, llm, prm)
